...
class LogWatcher(object):
    ...
    def __init__(self, input_file, callback, extensions=["log"], tail_lines=0,
                 sizehint=1048576):
        """Arguments:

        (str) @folder:
            the input_file folder or file to watch
    ...
    def handle_input(self, input_file):
        if os.path.isdir(input_file):
            self.folder = input_file
        else:
            self.file_name = os.path.basename(input_file)
            self.folder = os.path.dirname(input_file)
...
def callback_color(filename, lines):
    ...
    while lines:
        ...
        elif line[0:lv_len + 1] == 'Traceback':
            print(colored(line, 'red', attrs=['bold', 'blink', 'reverse']))
            in_traceback = True
            continue
        else:
            new_line = re.sub(r'(".*")', lambda m: colored(m.group(1), 'green'), line)
            print(new_line)
            continue
